...
def safe_fromstring(text, dtype=np.uint8):
    return np.frombuffer(text.encode(), dtype=dtype)

np.fromstring = safe_fromstring

import sys

import modal

# Evo2 docker image setup
evo2_image = (
    modal.Image.from_registry(
        "nvidia/cuda:12.4.0-devel-ubuntu22.04", add_python="3.12"
    )
    .apt_install(
        ["build-essential", "git", "cmake", "ninja-build", "libcudnn8", "libcudnn8-dev", "gcc", "g++"]
    )
    .env({
        "CC": "/usr/bin/gcc",
        "CXX": "/usr/bin/g++",
...
app = modal.App("helix-ai", image=evo2_image)
...
def run_brca1_analysis():
    ...
    # Rename columns for compatibility with Evo2
    ...
    # Build reference sequence index and variant sequence mappings
